...
*Eight planets, four rocky and four enormous, arranged in a pattern that turns out to be the fossil record of a catastrophe.*
...
**Exercise 1.** Venus and Earth are nearly the same size and formed in the same region of the solar nebula, yet Venus has a surface temperature of 465°C and Earth has an average of about 15°C. Prompt an LLM: "What is the runaway greenhouse effect, and why did it happen on Venus but not on Earth?" Evaluate whether the explanation correctly distinguishes the role of carbon dioxide concentration, ocean chemistry, and the absence of a carbonate-silicate cycle on Venus.

**Exercise 2.** A rock sample contains equal amounts of uranium-238 and lead-206. Uranium-238 has a half-life of 4.47 billion years. Prompt an LLM to calculate the age of the sample. Then ask it to explain what assumptions the calculation rests on. Does it identify the closed-system assumption — that no lead or uranium entered or left the rock since solidification? Evaluate the completeness of its answer.

**Exercise 3.** The Moon's surface has two visually distinct terrain types: heavily cratered highlands and smoother, darker maria. Prompt an LLM: "Why do the lunar highlands have more craters than the maria? What does this tell us about the relative ages of these surfaces?" Ask it to explain what crater counting can and cannot tell you. Does it correctly note that crater counting gives relative age, not absolute age in years?

**Exercise 4.** The Giant Impact Hypothesis proposes that the Moon formed from debris ejected when a Mars-sized body struck early Earth. Prompt an LLM to list the evidence for this hypothesis. Then ask: "What evidence would, if found, disprove or seriously challenge the Giant Impact Hypothesis?" Evaluate whether the LLM can engage with falsifiability, or whether it only recites supporting evidence.

**Exercise 5 (challenge).** Europa is one of the moons of Jupiter most often discussed as a candidate for extraterrestrial life. Prompt an LLM: "What is the evidence that Europa has a subsurface liquid ocean, and what energy source would sustain it?" Then ask: "What would be the three most important measurements a future Europa mission should make to determine whether the ocean could support life?" Evaluate whether the response is grounded in specific physical and chemical reasoning, or whether it gives generic answers about "the right conditions for life."
